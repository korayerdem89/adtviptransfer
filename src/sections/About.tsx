...
const valueIcons = [Shield, Clock, Target, Heart];

export default function About() {
  const { t, ts } = useLanguage();
  const sectionRef = useRef<HTMLDivElement>(null);

  const valuesData = t('about.values') as Array<{ title: string; description: string }> | undefined;
  const values = valueIcons.map((icon, i) => {
    if (!Array.isArray(valuesData)) return { icon, title: '', description: '' };
    return { icon, ...valuesData[i] };
  });

  const statsData = t('about.stats') as Array<{ value: string; label: string }> | undefined;
  const stats = Array.isArray(statsData) ? statsData : [];

  const featuresData = t('about.featuresList') as string[] | undefined;
  ...
  return (
    <section id="about" ref={sectionRef} className="relative py-24 bg-[#0a0a0a] overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="animate-on-scroll opacity-0 -translate-x-12 transition-all duration-700 relative">
            <div className="relative">
              ...
              <div className="relative rounded-2xl overflow-hidden">
                <img src="/about-team.jpg" alt="ADT VIP Transfer Team" className="w-full h-[400px] lg:h-[500px] object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
              </div>
              <div className="absolute -bottom-6 -right-6 bg-[#d4af37] text-black p-6 rounded-xl shadow-2xl">
                <Award className="w-10 h-10 mb-2" />
                <p className="text-2xl font-bold">10+</p>
                <p className="text-sm">{ts('about.yearsExperience')}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div>
              <div className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 inline-flex items-center gap-2 px-4 py-2 bg-[#d4af37]/10 border border-[#d4af37]/30 rounded-full mb-6">
                ...
                <span className="text-sm text-[#d4af37] tracking-wider uppercase">{ts('about.badge')}</span>
              </div>
              ...
              <h2 className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 delay-100 text-3xl sm:text-4xl lg:text-5xl font-serif font-bold text-white mb-6">
                {ts('about.title')} <span className="text-gradient-gold">{ts('about.titleHighlight')}</span>{' '}{ts('about.titleEnd')}
              </h2>

              <p className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 delay-200 text-gray-400 text-lg leading-relaxed">{ts('about.description')}</p>
            </div>

            <div className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 delay-300 space-y-4">
            ...
            <div className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 delay-400 grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6">
        ...
        <div className="mt-24">
          <div className="text-center mb-12">
            <h3 className="animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 text-2xl sm:text-3xl font-serif font-bold text-white">{ts('about.valuesTitle')}</h3>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((value, index) => (
              <div key={index} className={`animate-on-scroll opacity-0 translate-y-8 transition-all duration-700 delay-${(index + 5) * 100} group`}>
